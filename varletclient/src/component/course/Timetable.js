    ...
    if (isDelete) {
      try {
        const result = await jaxios.post(
          `/api/dayschedule/deleteDayschedule/${dseq}`,
        );
    ...
    <div className="timetable">
      <div className="courserow">
        <div
          ...
          style={{ backgroundColor: `#f2f2f2`, width: `${cellwidth / 2}%` }}
        ></div>
        {days.map((day, index) => (
          <div
            key={day}
            className="coursecell"
            ref={index === 0 ? courseCellRef : null}
            style={{ backgroundColor: `#f2f2f2`, width: `${cellwidth}%` }}
          >
            {day}
          </div>
        ))}
      ...
      {times.map((time) => (
        <div key={time} className="courserow">
          <div
            className="coursecell"
            style={{ backgroundColor: `#f8f8f8`, width: `${cellwidth / 2}%` }}
          >
            {time}
          </div>
          {days.map((_, index) => (
            <div
              key={index}
              className="coursecell"
              style={{ backgroundColor: `#f8f8f8`, width: `${cellwidth}%` }}
            ></div>
          ))}
        </div>
      ))}
      {daySchedule.length > 0 ? (
        dayschedule.map((contents, index) => (
          <div
            ...
            className="datacell"
            style={{
              right: `${cellposition[index].x * cellwidth}px`,
              top: `${cellposition[index].y * 40}px`,
              height: `${cellposition[index].l * 40}px`,
              backgroundColor: `${cellposition[index].c}`,
              color: "black",
              width: `${getCourseCellWidthInPixels()}px`,
            }}
            onClick={() =>
              scheduleDetail(contents.dseq, index, isScheduledetailVisible)
            }
          >
            {contents.dtitle}
