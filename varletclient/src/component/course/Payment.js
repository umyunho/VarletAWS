...
function Payment({ daySchedule }) {
      ...
      function (rsp) {
        ...
        if (rsp.success) {
          try {
            ...
            axios
              .post(`/api/pay/payment/${user.userid}`, {
  ...
  useEffect(() => {
    if (daySchedule && daySchedule.length > 0) {
      ...
      for (let i = 0; i < daySchedule.length; i++) {
        // dayschedule 대신 daySchedule을 사용
  ...
  return (
    <>
      <div
        className="rounded-lg border bg-card text-card-foreground shadow-sm w-full max-w-xs"
        data-v0-t="card"
      >
        ...
        <div className="p-6 grid gap-4">
            ...
            dayschedule.map((contents, index) => (
              <div className="grid gap-1">
                <div className="text-sm text-muted-foreground">
                  {contents.day_date.substring(5, 10)}
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium">
                    {contents.price} X {contents.pcount}명
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">
                  ...
                  <span className="text-sm font-medium">
                    {contents.pcount * contents.price}
                  </span>
                </div>
              </div>
            ))
          ) : (
            <div className="grid gap-1">결제할 목록이 없습니다.</div>
          )}
          <div
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-m text-muted-foreground">
              {totalPrice - usePoint >= 0 ? totalPrice - usePoint : 0}
            </span>
          </div>
        </div>
      </div>
