...
function Heading() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/member/getMyProfileImg/${userid}`, {
        params: {
  ...
  return (
    <div className="header">
      <div className="category_menu">
        ...
        <div
          className="head_contents"
          onClick={() => navigate("/contentsList")}
        >
          컨텐츠
        </div>
        <div className="rcommunity" onClick={() => navigate("/rcommunity")}>
          여행 코스 의뢰
        </div>
        <div className="reviewBoard" onClick={() => navigate("/reviewList")}>
          여행 후기
        </div>
        ...
        {!userCookie && (
          <div className="login-buttons">
            {!userCookie && (
              <div className="login" onClick={LoginWindow}>
                로그인
              </div>
            )}
            {!userCookie && <div className="line">|</div>}
            {!userCookie && (
              <div className="join" onClick={JoinWindow}>
                회원가입
              </div>
            )}
          </div>
      ...
      {isModalOpen && (
        ...
        >
          ...
          <div class="flex flex-col items-center w-80 h-full overflow-hidden text-gray-700 bg-gray-100 rounded">
            <a
              class="flex items-center justify-between w-full px-3 mt-3"
              href="#"
            >
              <div className="profileimg" onClick={handleNicknameClick}>
                <img src={profileImg} />
