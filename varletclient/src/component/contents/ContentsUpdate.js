...
function ContentsUpdate() {
  ...
  const getLocation2Index = (lnum, location2) => {
    ...
    if (locations2 && contents.location2) {
      const cityIndex = locations2.findIndex((item) =>
        location2.includes(item.label),
      );
  ...
  async function handleSubmit() {
    try {
      ...
      let result = await jaxios.post("/api/contents/updateContents", {
        cseq,
        ctype,
        cname,
        location,
  ...
  return (
    <>
      ...
      <div class="flex justify-center" style={{ marginTop: "80px" }}>
        <div
          class="rounded-lg border bg-card text-card-foreground shadow-sm w-full max-w-4xl"
          data-v0-t="card"
        >
          <div class="flex flex-col space-y-1.5 p-6">
            <h1 class="whitespace-nowrap font-semibold tracking-tight text-4xl">
              컨텐츠 수정
            </h1>
          </div>
          <div class="p-6 grid gap-8">
            ...
            <div class="grid gap-4">
              <label
                class="font-medium text-muted-foreground peer-disabled:cursor-not-allowed peer-disabled:opacity-70 text-lg"
                for="cost"
              >
                비용
              </label>
            ...
            <div className="field">
              ...
              <div>
                <img src={contentsimg} style={imgStyle} />
              </div>
            </div>
