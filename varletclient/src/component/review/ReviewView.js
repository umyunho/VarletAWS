...
function ReviewView() {
  ...
  useEffect(() => {
    // 리뷰 데이터 로드
    axios
      .get(`/api/review/getReviewView/${rseq}`)
      .then((result) => {
  ...
  function reviewDelete() {
    ...
    if (isConfirmed) {
      axios
        .delete(`/api/review/reviewDelete/${rseq}`)
        .then(() => {
          navigate("/reviewList");
        })
        .catch((err) => {
  ...
  function reviewEdit() {
    ...
    axios
      .post(`/api/review/updateReview/${rseq}`, formData, {
  ...
  function handleReplyDelete(replyId, replyUserId) {
    ...
    if (isConfirmed) {
      jaxios
        .delete(`/api/reply/deleteReply/${replyId}`, {
          data: { userId },
        })
        .then(() => {
          setReplies((prevReplies) =>
            prevReplies.filter((reply) => reply.renum !== replyId),
          );
        })
        .catch((err) => {
          console.error(err);
                    ...
                    {replies.length > 0 ? (
                      replies.map((reply) => (
                        <div key={reply.renum} className="border-b p-2">
                          <div>
                            <strong>{reply.userid || "Unknown"}</strong>
                          </div>
                          <div>{reply.content}</div>
                          <div>
                            {reply.writedate
                              ? new Date(reply.writedate).toLocaleString()
                              : "Unknown date"}
                          </div>
                          {reply.userid === userId && (
                            <button
                              onClick={() =>
                                handleReplyDelete(reply.renum, reply.userid)
                              }
