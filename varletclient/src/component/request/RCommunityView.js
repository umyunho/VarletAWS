...
function RCommunityView() {
  ...
  useEffect(() => {
    // 게시물 데이터 가져오기
    jaxios
      .get(`/api/rcommunity/rCommunityView/${rnum}`)
      .then((response) => {
  ...
  const fetchReplies = (pageNumber) => {
    ...
    jaxios
      .get(`/api/rcrecommend/getReplies/${rnum}`, {
  ...
  const deleteCommunity = (rnum) => {
    if (window.confirm("정말로 삭제하시겠습니까?")) {
      jaxios
        .delete(`/api/rcommunity/rCommunityDelete/${rnum}`)
        .then((response) => {
  ...
  const replyDelete = (rcnum) => {
    if (window.confirm("정말로 삭제하시겠습니까?")) {
      jaxios
        .delete(`/api/rcrecommend/deleteReply/${rcnum}`)
        .then(() => {
  ...
  const getLocationName = (location1, location2) => {
    const location1Name = location1Data[location1]
      ? location1Data[location1][0]
      : "정보 없음";
    const location2Options = location2Data[location1] || [];
    const location2Name =
      location2Options.find((option) => option.value === location2)?.label ||
      "정보 없음";
  ...
  const handlepicked = (rcnum) => {
    ...
    if (isConfirmed) {
      ...
      jaxios
        .post(`/api/rcrecommend/updateReplyPicked/${rcnum}`, { rpicked: "Y" })
        .then(() => {
          // 2. 게시글의 picked를 'Y'로 변경
          jaxios
            .post(`/api/rcommunity/updatePicked/${post.rnum}`, { picked: "Y" })
            .then(() => {
              alert("채택이 완료되었습니다.");
        ...
        <div className="bg-white bg-opacity-90 p-8  shadow-lg  min-h-screen mt-[80px]">
                    ...
                    <div className="flex-shrink-0">
                      ...
                      {showDates && (
                        <div className="text-xl mt-2">
                          ...
                          여행 종료일: {extractDate(post.enddate)} <br />총 여행
                          일수:{" "}
                          {getTravelDuration(post.startdate, post.enddate)}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <div class="border-t pt-6 mt-6">
            ...
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="w-6 h-6 text-blue-800 dark:text-blue-400"
                viewBox="0 0 24 24"
                strokeWidth="1.5"
                stroke="currentColor"
                fill="none"
                  ...
                  {replies.map((reply) => (
                    <li key={reply.rcnum} style={{ marginBottom: "20px" }}>
                      <div>
                        <div className="flex items-center gap-4 text-sm">
                          ...
                          <div className="flex items-center gap-2">
                            <div className="font-medium">
                              {maskeduser(reply.userid)}
                            </div>
                            <div className="text-muted-foreground">
                              {new Date(reply.writedate).toLocaleDateString(
                                "ko-KR",
                              )}
                            </div>
                          </div>
                        ...
                        {/* 숙소 이름 및 사진 */}
                        {reply.images &&
                          reply.images.filter((img) => img.imageType === "숙소")
                            .length > 0 && (
                            <div
                              style={{
                                display: "flex",
                      ...
                      {/* 콘텐츠 */}
                      <div>
                        <h2 className="text-2xl md:text-3xl font-middle">
                          상세 설명
                        </h2>
                        <h2 className="text-2xl md:text-3xl font-bold">
                          {reply.content}
                      ...
                      {/* 기타 사진 */}
                      {reply.images &&
                        reply.images.filter((img) => img.imageType === "기타")
                          .length > 0 && (
                          <div>
                            <h3 className="text-xl font-bold">기타 사진:</h3>
                            <div style={{ display: "flex", flexWrap: "wrap" }}>
                              {reply.images
                                .filter((img) => img.imageType === "기타")
                                .map((image) => (
                                  <img
                                    key={image.id}
                                    src={`${image.filePath}`} // S3 URL 조합
                    ...
                    (_, index) => (
                      <li key={startPage + index}>
                        <a
                          ...
                          aria-current={
                            page === startPage + index ? "page" : undefined
                          }
                        >
                          {startPage + index}
                        </a>
