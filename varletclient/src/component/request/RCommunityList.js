          ...
          <ul>
            ...
            {posts.length > 0 ? (
              posts.map((post, index) => (
                <li
                  key={post.rnum}
                  className="flex items-center justify-center border-b border-gray-300 py-2 h-20"
                >
                  <span className="w-1/12 text-center bg-blue-500 text-white rounded-full px-2 py-1 text-lg">
                    {post.rnum}
                  </span>
                  <span
                    className="w-4/12 text-left cursor-pointer text-blue-500 text-lg"
                    onClick={() => {
                      logCheck(`/RcommunityView/${post.rnum}`);
                    }}
                  >
                    {post.title}[{post.replyCount}]
                  </span>
                  <span className="w-2/12 text-center text-lg">
                    {location1Data[post.location]}
                  </span>
                  <span className="w-2/12 text-center text-lg">
                    {location2Data[post.location]?.find(
                      (item) => item.value === post.location2,
                    )?.label || "전체"}
                  </span>
                  <span className="w-2/12 text-center text-lg">
                  ...
                  <span className="w-1/12 text-center text-lg">
                    {post.views}
                  </span>
                </li>
              ))
