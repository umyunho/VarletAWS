...
function House() {
      ...
      if (Array.isArray(newHouses) && newHouses.length > 0) {
        setHouse((prevHouse) => {
          ...
          const uniqueContents = Array.from(
            new Set(combinedHouse.map((house) => house.hseq)),
  ...
  const filterContents = useCallback(async () => {
    if (searchTerm.trim() === "") {
      ...
      const result = await axios.get("/api/house/search", {
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <p className="text-muted-foreground mb-4">
                    {house.indate.substring(0, 10)}
                  </p>
                  <div className="flex items-center justify-between">
                    <span className="text-primary font-semibold">
                      {house.cost == 0 ? "무료" : house.cost}
                    </span>
                  </div>
                </div>
              </div>
