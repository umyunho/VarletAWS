...
function Notice() {
  ...
  function onPageMove(page) {
    // 페이지 표시방식
    axios
      .get(`/api/notice/noticeList/${page}`)
      .then((result) => {
        setNoticeList([...result.data.noticeList]);
        setPaging(result.data.paging);
        ...
        <div className="NoticeList" style={{ flex: "4" }}>
          ...
          <div className="noticeTable">
            <div className="notice_row">
              <div className="notice_col" style={{ flex: "2.6" }}>
                번호
              </div>
              <div className="notice_col" style={{ flex: "4" }}>
                제목
              </div>
              <div className="notice_col" style={{ flex: "2" }}>
                등록일
              </div>
            </div>
          </div>
            ...
            ? noticeList.map((notice, idx) => {
                return (
                  <div className="notice_row2" key={idx}>
                    ...
                    <div
                      className="notice_coll"
                      style={{ flex: "4" }}
                      onClick={() => {
                        onNoticeView(notice.nseq);
                      }}
                    >
                      {" "}
                      {notice.subject}
