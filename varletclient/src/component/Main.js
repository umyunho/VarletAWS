...
function Main() {
  ...
  const loadReviews = useCallback(async (pageNumber) => {
    try {
      ...
      const filteredReviews = newReviews.filter(
        (review) => review.readcount >= 200,
      );

      if (filteredReviews.length > 0) {
        setReviews((prevReviews) => {
          // 최신 5개 리뷰만 유지
          const sortedReviews = [...prevReviews, ...filteredReviews].sort(
  ...
  useEffect(() => {
    const result = axios
      .get("/api/contents/recentContentsList")
      .then((result) => {
  ...
  return (
    <>
      ...
      <div className="flex min-h-[100dvh] flex-col w-full">
        <section
          className="w-full bg-gradient-to-r from-[white] to-[white] py-20 md:py-15"
          style={{
      ...
      <div className="flex flex-col items-center w-full">
        ...
        <div
          ...
          className="flex gap-6 w-full p-4 overflow-x-hidden"
        >
          <div className="relative overflow-hidden rounded-lg flex-shrink-0 w-[600px]">
            ...
            <div className="mt-2 text-lg font-bold">
              세계여행 대신 빡세게 국내여행 - 전라남도 해남으로 출발 01
            </div>
          </div>
          <div className="relative overflow-hidden rounded-lg flex-shrink-0 w-[600px]">
            <iframe
            ...
            ></iframe>
            <div className="mt-2 text-lg font-bold">
              제주여행 vlogㅣ이렇게 여행 와서 잘 먹고 힐링하려고 열심히
              산거지!!🍀ㅣ제주동쪽 감성숙소🏝️ㅣ(고사리파스타, 돔베고기,
              제주디저트맛집들..♥︎)
            </div>
          </div>
          <div className="relative overflow-hidden rounded-lg flex-shrink-0 w-[600px]">
            ...
            ></iframe>
            <div className="mt-2 text-lg font-bold">
              민이수 1년 anniversary 부산여행 VLOG ❣️ | 극악무도한 P들의 나름
              스무스 했던 무계획 1박2일 여행 [ENG]
            </div>
          </div>
          <div className="relative overflow-hidden rounded-lg flex-shrink-0 w-[600px]">
            <iframe
